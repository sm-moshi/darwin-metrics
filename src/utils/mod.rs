...
mod tests {
    ...
    fn test_raw_str_to_string() {
        ...
        unsafe {
            assert_eq!(
                raw_str_to_string(test_str.as_ptr(), 4),
                Some("test".to_string())
            );
            assert_eq!(raw_str_to_string(test_str.as_ptr(), 0), None);
            assert_eq!(raw_str_to_string(std::ptr::null(), 4), None);
        }
    }
    ...
    fn test_raw_f64_slice_to_vec() {
        ...
        unsafe {
            assert_eq!(
                raw_f64_slice_to_vec(test_array.as_ptr(), 3),
                Some(test_array.clone())
            );
            assert_eq!(raw_f64_slice_to_vec(test_array.as_ptr(), 0), None);
