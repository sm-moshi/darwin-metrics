...
use crate::utils::bindings::{
    IOByteCount, SMCKeyData_t, smc_key_from_chars,
    SMC_KEY_CPU_TEMP, SMC_KEY_GPU_TEMP, SMC_KEY_FAN_SPEED,
    KERNEL_INDEX_SMC, SMC_CMD_READ_BYTES, SMC_CMD_READ_KEYINFO,
    kIOReturnSuccess,
    IOServiceGetMatchingService, IOServiceMatching, IOServiceOpen,
    IOServiceClose, IORegistryEntryCreateCFProperties, IOConnectCallStructMethod
};
use std::ffi::CString;
use std::mem::size_of;
...
impl IOKitImpl {
    fn smc_read_key(&self, key: [c_char; 4]) -> Result<f64> {
        unsafe {
            ...
            Err(Error::invalid_data(format!("Unsupported SMC data type: {:?}", std::str::from_utf8(&data_type).unwrap_or("Unknown"))))
        }
    }
}
...
impl IOKit for IOKitImpl {
    fn io_service_matching(&self, service_name: &str) -> Retained<NSDictionary<NSString, NSObject>> {
        autoreleasepool(|_| {
            unsafe {
                let empty_dict = Retained::from_raw(msg_send![class!(NSDictionary), dictionary]).expect("Failed to create dictionary");
    ...
    fn io_registry_entry_create_cf_properties(&self, entry: &AnyObject) -> Result<Retained<NSDictionary<NSString, NSObject>>> {
        ...
        autoreleasepool(|_| {
            unsafe {
                ...
                Retained::from_raw(dict_ptr).ok_or_else(|| Error::system("Failed to retain properties"))
            }
        })
    }
    ...
    fn get_string_property(&self, dict: &NSDictionary<NSString, NSObject>, key: &str) -> Option<String> {
        ...
        unsafe { dict.valueForKey(&key).and_then(|obj| obj.downcast::<NSString>().ok()).map(|s| s.to_string()) }
    }

    fn get_number_property(&self, dict: &NSDictionary<NSString, NSObject>, key: &str) -> Option<i64> {
        let key = NSString::from_str(key);
        unsafe { dict.valueForKey(&key).and_then(|obj| obj.downcast::<NSNumber>().ok()).map(|n| n.as_i64()) }
    }

    fn get_bool_property(&self, dict: &NSDictionary<NSString, NSObject>, key: &str) -> Option<bool> {
        let key = NSString::from_str(key);
        unsafe { dict.valueForKey(&key).and_then(|obj| obj.downcast::<NSNumber>().ok()).map(|n| n.as_bool()) }
    }
        ...
        autoreleasepool(|_| {
            ...
            if let Some(agpm_service) = self.io_service_get_matching_service(&agpm_matching) {
                if let Ok(properties) = self.io_registry_entry_create_cf_properties(&agpm_service) {
                    ...
                    if let Some(perf_threshold) = self.get_number_property(&properties, "GPUPerfThreshold") {
            ...
            if stats.memory_total == 0 {
                ...
                if let Some(system) = self.io_service_get_matching_service(&system_matching) {
                    if let Ok(properties) = self.io_registry_entry_create_cf_properties(&system) {
                        if let Some(memory) = self.get_number_property(&properties, "total-ram-size") {
                            ...
                            stats.memory_used = ((stats.utilization / 100.0) * stats.memory_total as f64) as u64;
                        }
                    }
                }
            }
