...
use darwin_metrics::traits::CpuMonitor;
    ...
    loop {
        ...
        for interface in manager.interfaces() {
            ...
            if recv_err_rate > 0.0 || send_err_rate > 0.0 {
                println!("  Error rates: {:.6} receive, {:.6} send", 
                    recv_err_rate, 
                    send_err_rate);
            }
            ...
            println!("  Packet receive: {}", create_ascii_graph(recv_percent.min(100.0), graph_width));
            println!("  Packet send:    {}", create_ascii_graph(sent_percent.min(100.0), graph_width));
            println!();
        }
