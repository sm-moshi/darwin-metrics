...
fn test_health_monitor() -> Result<()> {
    let battery = TestBatteryBuilder::new().cycle_count(100).capacity(8000.0, 10000.0).build()?;
...
fn test_health_monitor_zero_capacity() -> Result<()> {
    let battery = TestBatteryBuilder::new().cycle_count(100).capacity(0.0, 10000.0).build()?;
