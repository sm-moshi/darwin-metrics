...
async fn test_utilization_monitor_with_nonexistent_disk() {
    ...
    assert_eq!(
        utilization_result.unwrap(),
        0.0,
        "Nonexistent disk should report 0% utilization"
    );
}
...
async fn test_utilization_monitor_updates() {
    ...
    for _ in 0..3 {
        ...
        assert!(
            utilization >= 0.0 && utilization <= 100.0,
            "Utilization should always be between 0 and 100%"
        );
...
async fn test_utilization_monitor_stress() {
    ...
    for _ in 0..10 {
        ...
        assert!(
            utilization >= 0.0 && utilization <= 100.0,
            "Utilization should remain valid under stress"
        );
