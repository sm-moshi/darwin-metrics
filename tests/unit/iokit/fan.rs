use darwin_metrics::{
    error::Error,
    hardware::iokit::{FanInfo, IOKit, IOKitImpl, MockIOKit},
};
...
fn test_get_fan_info() {
    ...
    mock_iokit
        .expect_get_fan_info()
        .returning(|_| Ok(FanInfo { speed_rpm: 2000, min_speed: 500, max_speed: 5000, percentage: 40.0 }));
...
fn test_get_fan_info_min_max_equal() {
    ...
    mock_iokit
        .expect_get_fan_info()
        .returning(|_| Ok(FanInfo { speed_rpm: 2000, min_speed: 2000, max_speed: 2000, percentage: 0.0 }));
...
fn test_fan_percentage_calculation() {
    ...
    let fan_info = FanInfo { speed_rpm: speed, min_speed: min, max_speed: max, percentage: expected_percentage };
...
fn test_fan_percentage_limits() {
    ...
    let zero_max_fan = FanInfo { speed_rpm: 2000, min_speed: 1000, max_speed: 0, percentage: 0.0 };
...
fn test_fan_info_edge_cases() {
    ...
    mock.expect_get_fan_info().returning(|_| Ok(FanInfo { speed_rpm: 0, min_speed: 0, max_speed: 0, percentage: 0.0 }));
